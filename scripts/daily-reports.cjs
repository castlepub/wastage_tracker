...
async function main() {
  ...
  <body>
    <div class="container">
      ...
      <div class="summary">
        <p><strong>Period:</strong> ${config.startDate.format('DD.MM.YYYY HH:mm')} - ${config.endDate.format('DD.MM.YYYY HH:mm')} UTC</p>
        <p><strong>Total Entries:</strong> ${validEntries.length}</p>
        <p><strong>Total Cost:</strong> <span class="cost">€${validEntries.reduce((sum, e) => sum + (e.total_cost || 0), 0).toFixed(2)}</span></p>
      </div>
...
`;
    ...
    // After generating the HTML content, send via Resend
    const emailData = {
